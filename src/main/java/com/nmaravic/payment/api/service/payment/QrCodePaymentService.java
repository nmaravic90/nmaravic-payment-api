...
@Service
public class QrCodePaymentService extends AbstractPaymentService<QrCodePaymentRequest> {

    public QrCodePaymentService(IdempotencyService idempotencyService,
    ...
    @Override
    @Transactional
    public TransactionResponse process(UUID idempotencyKey, QrCodePaymentRequest request) {
    ...
    @Override
    protected void validateAndDeduct(QrCodePaymentRequest request) {
        balanceService.validateAndSubtractBalance(request.getUserId(), BigDecimal.valueOf(request.getAmount()));
    }

    @Override
    protected Transaction toTransaction(QrCodePaymentRequest request) {
